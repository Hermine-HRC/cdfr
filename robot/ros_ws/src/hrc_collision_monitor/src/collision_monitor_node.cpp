  ...
  std::string & state_topic)
{
  ...
  if (
    !configureSources(
      base_frame_id_, odom_frame_id, transform_tolerance_, source_timeout, base_shift_correction))
  {
    return false;
  }
...
void CollisionMonitor::process(const Velocity & cmd_vel_in, const std_msgs::msg::Header & header)
{
  ...
  if (!no_source_poly_ && !nav2_util::getTransform(
      base_frame_id_, source_base_frame_id_,
  ...
  for (std::shared_ptr<Source> source : sources_) {
    ...
    if (source->getEnabled()) {
      ...
      if (!no_source_poly_) {
        std::vector<Point> points_in_source_polygon;
        for (const auto & point : iter.first->second) {
          tf2::Vector3 p_v3_b(point.x, point.y, 0.0);
          tf2::Vector3 p_v3_s = tf_transform * p_v3_b;
          for (const std::shared_ptr<Polygon> & polygon : polygons_) {
            if (polygon->isSource() && polygon->isPointInside({p_v3_s.x(), p_v3_s.y()})) {
              points_in_source_polygon.push_back(point);
              break;
            }
          }
        }
        iter.first->second = points_in_source_polygon;
      }
    }
  ...
  for (std::shared_ptr<Polygon> polygon : polygons_) {
    if (!polygon->getEnabled() || polygon->isSource()) {
      continue;
    }
