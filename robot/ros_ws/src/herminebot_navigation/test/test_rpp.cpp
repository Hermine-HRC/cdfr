...
class BasicAPIRPP : public hrc_rpp_controller::RegulatedPurePursuitController
{
public:
    BasicAPIRPP() : hrc_rpp_controller::RegulatedPurePursuitController() {}
...
TEST(RegulatedPurePursuitTest, testDynamicParameter)
{
    ...
    costmap->on_configure(rclcpp_lifecycle::State());
    auto ctrl =
        std::make_unique<hrc_rpp_controller::RegulatedPurePursuitController>();
    auto tf = std::make_shared<tf2_ros::Buffer>(node->get_clock());
