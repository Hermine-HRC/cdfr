...
bool RegulatedPurePursuitController::useReverse() const
{
  ...
  if (path_handler_->getPlan().poses.size() < 10) {return false;}

  const geometry_msgs::msg::PoseStamped goal_pose = path_handler_->getPlan().poses.back();
  const geometry_msgs::msg::PoseStamped prev_goal_pose =
    path_handler_->getPlan().poses.at(path_handler_->getPlan().poses.size() - 10);
  ...
  const bool final_heading)
{
  ...
  angular_vel = sign *
    (final_heading ? params_->rotate_to_heading_angular_vel : params_->rotate_to_path_angular_vel);
