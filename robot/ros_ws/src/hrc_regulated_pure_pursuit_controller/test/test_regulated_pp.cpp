...
#include "hrc_regulated_pure_pursuit_controller/regulated_pure_pursuit_controller.hpp"
...
class BasicAPIRPP : public hrc_regulated_pure_pursuit_controller::RegulatedPurePursuitController
{
public:
  BasicAPIRPP()
  : hrc_regulated_pure_pursuit_controller::RegulatedPurePursuitController() {}
...
TEST(RegulatedPurePursuitTest, testDynamicParameter)
{
  ...
  costmap->on_configure(rclcpp_lifecycle::State());
  auto ctrl =
    std::make_unique<hrc_regulated_pure_pursuit_controller::RegulatedPurePursuitController>();
  auto tf = std::make_shared<tf2_ros::Buffer>(node->get_clock());
...
TEST_F(TransformGlobalPlanTest, rotate_to_path_rotate_to_heading_and_allow_reversing)
{
  ...
  nav2_util::declare_parameter_if_not_declared(node_,
    "test_rpp.use_rotate_to_heading", rclcpp::ParameterValue(true));
  nav2_util::declare_parameter_if_not_declared(node_,
    "test_rpp.allow_reversing", rclcpp::ParameterValue(true));
  nav2_util::declare_parameter_if_not_declared(
  ...
  carrot.pose.position.x = -0.2;
  carrot.pose.position.y = 0.0;
  EXPECT_EQ(ctrl_->shouldRotateToPathWrapper(carrot, angle_to_path_rtn, -1.0), false);
  EXPECT_NEAR(angle_to_path_rtn, 0.0, 0.0001);
  ...
  carrot.pose.position.x = -0.2;
  carrot.pose.position.y = 0.0;
  EXPECT_EQ(ctrl_->shouldRotateToPathWrapper(carrot, angle_to_path_rtn, -1.0), true);
